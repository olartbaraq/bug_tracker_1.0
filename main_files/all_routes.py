...
from main_files import db
from flask import render_template, url_for, redirect, request, flash, jsonify
from flask_login import login_user, logout_user, login_required, current_user
from main_files.forms_input import (
    Project_Details,
...
def projects_page():
    ...
    if form3.validate_on_submit():
        ...
        searched_projects = searched_projects.filter(
            Project.project_description.like("%" + ad_search + "%")
        )
...
@bp.route("/project-details", methods=["GET", "POST"])
...
@bp.route("/project-details/edit/<int:project_id>", methods=["GET", "POST"])
def edit_projects(project_id):
    ...
    return render_template(
        "edit-individual-project.html",
        form=form,
        form2=form2,
        indy_project=indy_project,
        project_id=indy_project.project_id,
        all_projects=all_projects,
    )


@bp.route("/projects/delete/<int:project_id>", methods=["GET", "DELETE"])
...
def users_page():
    ...
    if form3.validate_on_submit():
        ...
        searched_users = searched_users.filter(
            User.assigned_project.like("%" + ad_search + "%")
        )
...
@bp.route("/User-Details", methods=["GET", "POST"])
def user_info_page():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            role_to_create = Role(
                assigned_role=user_to_create.user_roles,
...
@bp.route("/user-info/edit/<int:user_id>", methods=["GET", "POST"])
...
@bp.route("/users/delete/<int:user_id>", methods=["GET", "DELETE"])
...
def issues_page():
    ...
    if form.validate_on_submit():
        ...
        searched_issues = searched_issues.filter(
            Issues.issue_summary.like("%" + ad_search + "%")
        )
        ...
        return render_template(
            ...
            form=form,
            searched=ad_search,
            searched_issues=searched_issues,
        )
    return render_template("issues.html", all_issues=all_issues, form=form)


@bp.route("/Isssue-Details", methods=["GET", "POST"])
def issue_info_page():
    """a route to return the website users details"""
    form = Issue_Details()

    form.related_project.choices = ["<Select Project>"] + [
        (project.project_name) for project in Project.query.all()
    ]
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            return redirect(url_for("main.issues_page"))
    if form.errors != {}:
        for err_msg in form.errors.values():
            flash("Error: {}".format(err_msg), category="danger")
...
@bp.route("/issue-info/edit/<int:issue_id>", methods=["GET", "POST"])
def edit_issue_page(issue_id):
    ...
    form.identified_by.data = User.query.filter_by(
        username=each_issue.identified_by_person_id
    ).first()
    ...
    return render_template(
        "issue-details-edit.html",
        form=form,
        form2=form2,
        issue_id=each_issue.id,
        each_issue=each_issue,
        Issue_name=Issue_name,
        Issue_proj=Issue_proj,
    )


@bp.route("/issues/delete/<int:issue_id>", methods=["GET", "DELETE"])
def delete_issue_page(issue_id):
    ...
    try:
        ...
        return render_template(
            "issues.html", all_issues=all_issues, each_issue=each_issue
        )
...
@bp.route("/issue-summary", methods=["GET", "POST"])
def issue_summary_page():
    """ """
    form4 = Issue_Summary()
    form4.related_project.choices = [
        (project.project_name) for project in Project.query.all()
    ]

    Issue_proj = Project.query.filter_by(
        project_name=form4.related_project.data
    ).first()
    if request.method == "POST":
